...
const FOOTER_GROUPS: { title: string; links: { label: string; to?: string; href?: string }[] }[] = [
  ...
  },
  {
    title: "Platform",
    links: [
      { label: "Health endpoint", href: `${siteUrl}/health` },
      { label: "Hosted apps", href: siteUrl },
    ],
  },
];
...
export function LandingPage() {
  ...
  useSeo({
    title: "LocalMe — Ship a full-stack web app with frontend code only",
    description:
  ...
  const stats = [
    { label: "Accounts", value: info?.stats.accounts },
    { label: "Projects", value: info?.stats.projects },
    { label: "Live apps", value: info?.stats.deployedApps },
  ];
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-40 border-b border-border bg-background/85 backdrop-blur supports-[backdrop-filter]:bg-background/70">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-5">
          <Link to="/" aria-label="LocalMe home">
            <BrandMark />
          </Link>
