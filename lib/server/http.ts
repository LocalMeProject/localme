...
export async function parseJson<S extends z.ZodTypeAny>(
  ...
  try {
    ...
    return { ok: false, response: apiError("bad_request", "Body must be valid JSON.") };
  }
  ...
  if (!result.success) {
    ...
    return {
      ok: false,
      response: apiError("bad_request", `Invalid body${where}: ${first?.message ?? "validation failed"}`),
    };
  }
...
export function handler(fn: (request: Request) => Promise<NextResponse>) {
